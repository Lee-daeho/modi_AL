...
# Custom
from config import *
from models.query_models import VAE, Discriminator, GCN, TVAE, TDiscriminator
...
def train_ta_vaal(models, optimizers, labeled_dataloader, unlabeled_dataloader, cycle):
...
def query_samples(model, method, data_unlabeled, subset, labeled_set, cycle, args):
    ...
    if method == 'TA-VAAL':
        ...
        if args.dataset == 'fashionmnist':
            vae = TVAE(28,1,3)
            discriminator = TDiscriminator(28)
        else:
            vae = TVAE()
            discriminator = TDiscriminator(32)
        ...
        train_ta_vaal(models,optimizers, labeled_loader, unlabeled_loader, cycle+1)
        task_model = models['backbone']
        ranker = models['module']        
        all_preds, all_indices = [], []
